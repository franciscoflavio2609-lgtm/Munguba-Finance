...
const VIEW_LABELS = {
  ...
  inv:'Investimentos', calc:'Calculadoras', radar:'Radar Econômico',
  ach:'Conquistas', fam:'Família', drm:'Sonhos', edu:'Educação', ecomp:'Psicologia Financeira',
  chat:'Assistente IA', faq:'Tira-Dúvidas', more:'Menu'
}
...
function AppShell() {
  ...
  useEffect(() => {
    if (!state.loading) {
      if (state.session) {
        // Verifica se é primeira vez (sem transações E sem flag de onboarding)
        const hasOnboarded = localStorage.getItem(`munguba_onboarded_${state.session.user.id}`)
        if (!hasOnboarded) {
          setScreen('onboarding')
        } else {
          setScreen('app')
        }
      } else if (screen === 'app' || screen === 'onboarding') setScreen('splash')
    }
  ...
  if (screen === 'onboarding') return <OnboardingScreen userId={state.session.user.id} onComplete={() => {
    localStorage.setItem(`munguba_onboarded_${state.session.user.id}`, '1')
    setScreen('app')
  }} />

  const VIEWS = {
    dash: Dashboard, txn: TransactionsScreen, bud: BudgetScreen,
    inv: InvestmentScreen, calc: CalculatorsScreen, radar: RadarScreen,
    ach: AchievementsScreen, fam: FamilyScreen, drm: DreamsScreen, edu: EducationScreen,
