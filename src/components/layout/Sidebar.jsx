...
const NAV_ITEMS = [
  ...
  { id:'calc', label:'Calculadoras', group:'main', color:'#0E7490',
    icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><line x1="8" y1="10" x2="8" y2="10"/><line x1="12" y1="10" x2="12" y2="10"/><line x1="16" y1="10" x2="16" y2="10"/><line x1="8" y1="14" x2="8" y2="14"/><line x1="12" y1="14" x2="12" y2="14"/><line x1="16" y1="14" x2="16" y2="14"/><line x1="8" y1="18" x2="8" y2="18"/><line x1="12" y1="18" x2="12" y2="18"/><line x1="16" y1="18" x2="16" y2="18"/></svg>},
  { id:'radar', label:'Radar Econômico', group:'main', color:'#7C3AED',
    icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><circle cx="12" cy="12" r="2"/><path d="M16.24 7.76a6 6 0 0 1 0 8.49m-8.48-.01a6 6 0 0 1 0-8.49m11.31-2.82a10 10 0 0 1 0 14.14m-14.14 0a10 10 0 0 1 0-14.14"/></svg>},
  { id:'ach', label:'Conquistas', group:'main', color:'#D97706',
  ...
  { id:'fam', label:'Família', group:'main', color:'#EC4899',
    icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>},
...
const STORAGE_KEY = 'munguba_sidebar_wide'
...
export default function Sidebar({ activeView, onNav, onAdd, onReport, onLogout }) {
  ...
  const [wide, setWide] = useState(() => {
    try {
      ...
      return saved !== null ? saved === '1' : true // padrão: expandida na primeira visita
    } catch { return true }
  })
  ...
  return (
    <nav style={{
      ...
      {/* Logo + botão de expandir/recolher (visível e claro) */}
      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:16, width:'100%' }}>
        <div style={{
          display:'flex',alignItems:'center',gap:10,flex:1,minWidth:0,
          padding: wide?'8px 8px 8px 14px':'8px 0',
          borderRadius:14,width:'100%',
          justifyContent: wide ? 'flex-start' : 'center'
        }}>
          <MungubaTree size={wide ? 40 : 46} />
          {wide && (
            <div style={{display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
              <span style={{fontSize:15,fontWeight:800,color:'#fff',letterSpacing:'-.01em',lineHeight:1.2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>Munguba Finance</span>
              <span style={{fontSize:8.5,color:'rgba(255,255,255,.42)',letterSpacing:'.12em',textTransform:'uppercase',marginTop:3,whiteSpace:'nowrap'}}>Seu Parceiro Financeiro</span>
            </div>
          )}
        </div>
        {/* Botão de expandir/recolher — sempre visível, com seta clara */}
        <button
          onClick={toggleWide}
          title={wide ? 'Recolher menu' : 'Expandir menu'}
          ...
          style={{
            ...
            marginRight: wide ? 4 : 0
          }}
          onMouseOver={e=>{e.currentTarget.style.background='rgba(255,255,255,.2)';e.currentTarget.style.color='#fff'}}
          onMouseOut={e=>{e.currentTarget.style.background='rgba(255,255,255,.1)';e.currentTarget.style.color='rgba(255,255,255,.65)'}}
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transform: wide ? 'rotate(180deg)' : 'none', transition:'transform .28s' }}>
      ...
      <button onClick={onAdd} style={{
        ...
        background:'rgba(255,255,255,.12)',marginBottom:8,flexShrink:0,
      ...
      <button onClick={onReport} style={{
        ...
        padding: wide?'0 12px':0,
        borderRadius:10,cursor:'pointer',border:'none',
        background:'transparent',marginBottom:12,flexShrink:0,
        fontFamily:'Outfit,sans-serif',fontSize:13,fontWeight:600,
      ...
      {/* Nav groups — com scroll interno para não cortar itens em telas menores */}
      <div className="no-scrollbar" style={{ flex:1, minHeight:0, overflowY:'auto', overflowX:'hidden', width:'100%' }}>
        {['main','tools'].map(group=>(
          <div key={group} style={{width:'100%',display:'flex',flexDirection:'column',gap:2,marginBottom:4}}>
            ...
            {NAV_ITEMS.filter(n=>n.group===group).map(item=>(
              <button key={item.id} onClick={()=>onNav(item.id)} title={!wide ? item.label : undefined} style={{
              ...
              onMouseOut={e=>{if(activeView!==item.id)e.currentTarget.style.background='transparent'}}>
                ...
                {wide && <span style={{overflow:'hidden',textOverflow:'ellipsis'}}>{item.label}</span>}
                {item.badge && (
                  <div style={{
      ...
      onMouseOut={e=>e.currentTarget.style.background='rgba(255,255,255,.07)'}>
        ...
        {wide && (
          <div style={{minWidth:0,overflow:'hidden'}}>
            <div style={{fontSize:12,color:'#fff',fontWeight:700,lineHeight:1.3,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{userName}</div>
      ...
      <div style={{ flexShrink:0 }}>
        <LogoutBtn wide={wide} onLogout={onLogout}/>
      </div>
    </nav>
  )
}
