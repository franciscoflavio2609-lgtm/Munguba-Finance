...
export default function InvestmentScreen() {
  ...
  const save = async () => {
    ...
    setSalvando(true)
    try {
      await addInvestment({ ...form, val: parseFloat(form.val), rt: parseFloat(form.rt||0) })
      setForm({ tp:'', nm:'', val:'', rt:'', date: new Date().toISOString().split('T')[0] })
      setTab('portfolio')
    } catch (e) {
      alert('Erro ao salvar o investimento. Tente novamente.')
    } finally {
      setSalvando(false)
    }
  }
      ...
      {tab === 'add' && (
        <div style={card}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <button onClick={save} disabled={salvando} style={{ padding:14, borderRadius:14, border:'none', background:'#076B3E', color:'#fff', fontFamily:'Outfit,sans-serif', fontSize:14, fontWeight:700, cursor: salvando ? 'wait' : 'pointer', opacity: salvando ? 0.7 : 1 }}>{salvando ? '⏳ Salvando...' : '✓ Registrar Investimento'}</button>
          </div>
        </div>
      )}
    </div>
