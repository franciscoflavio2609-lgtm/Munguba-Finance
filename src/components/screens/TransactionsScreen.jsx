...
import { exportarTransacoesCSV, exportarExtratoPDF } from '../../lib/exportUtils'
...
export default function TransactionsScreen() {
  ...
  const handleExportCSV = () => {
    exportarTransacoesCSV(filtered.length > 0 ? filtered : state.transactions)
    setShowExport(false)
  }
  const handleExportPDF = () => {
    exportarExtratoPDF(filtered.length > 0 ? filtered : state.transactions, nomeUsuario)
    setShowExport(false)
  }
        ...
        <div style={{ display:'flex', gap:10, marginBottom:18, flexWrap:'wrap' }}>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={() => setShowExport(!showExport)} style={{
              padding:'8px 14px', borderRadius:10, border:'1.5px solid rgba(10,138,82,.25)',
              background:'#F0F9F4', color:'#076B3E',
              fontFamily:'Outfit,sans-serif', fontSize:12, fontWeight:700, cursor:'pointer',
            ...
            {showExport && (
              <div style={{
                ...
                background:'#fff', borderRadius:14, boxShadow:'0 8px 24px rgba(4,77,44,.18)',
                border:'1px solid rgba(10,138,82,.15)', padding:8, minWidth:180
              }}>
                <button onClick={handleExportPDF} style={{
                  display:'flex', alignItems:'center', gap:8, width:'100%',
                  padding:'10px 12px', borderRadius:10, border:'none', background:'transparent',
                  fontFamily:'Outfit,sans-serif', fontSize:12.5, fontWeight:600, color:'#0D1F17',
