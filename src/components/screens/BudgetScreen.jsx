...
export default function BudgetScreen() {
  ...
  const storageKey = userId ? `munguba_budget_${userId}` : null
  ...
  // Recarrega os limites se o usuário mudar (ex: logout/login de outra conta)
  useEffect(() => {
    if (!storageKey) return
    try {
      const saved = localStorage.getItem(storageKey)
      setLimits(saved ? JSON.parse(saved) : {})
    } catch { setLimits({}) }
  }, [storageKey])

  // Salva automaticamente sempre que os limites mudarem
  useEffect(() => {
    if (!storageKey) return
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: '#fff', borderRadius: 24, padding: 24, width: '100%', maxWidth: 480, maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                ...
                <button onClick={() => { dispatch({ type:'SET_BUDGET_CONFIGURED' }); setShowSetup(false) }} style={{ flex: 1, padding: 12, borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#044D2C,#076B3E)', color: '#fff', fontFamily: 'Outfit,sans-serif', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Salvar orçamento</button>
              </div>
            </div>
          </div>
        )}
