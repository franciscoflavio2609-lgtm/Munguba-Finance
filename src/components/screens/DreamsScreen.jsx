...
export default function DreamsScreen() {
  const { state, addDream, depositDream } = useApp()
  ...
  const save = async () => {
    ...
    setSalvando(true)
    try {
      await addDream({ ...form, target: parseFloat(form.target) })
      setForm({ name:'', icon:'✈️', target:'', why:'', banner:'bn-green' })
      setAdding(false)
    } catch (e) {
      alert('Erro ao criar o sonho. Tente novamente.')
    } finally {
      setSalvando(false)
    }
  }
  ...
  const deposit = async (idx) => {
    ...
    const sonho = state.dreams[idx]
    if (!sonho) return
    const novoTotal = Number(sonho.current || 0) + amt
    setDepositando(true)
    try {
      await depositDream(sonho.id, novoTotal)
      setDepositing(null)
      setDepAmt('')
    } catch (e) {
      alert('Erro ao registrar o depósito. Tente novamente.')
    } finally {
      ...
      {adding && (
        <div style={{ ...card, marginBottom:18, border:'1.5px solid rgba(10,138,82,.3)' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            <button onClick={save} disabled={salvando} style={{ padding:13, borderRadius:14, border:'none', background:'#076B3E', color:'#fff', fontFamily:'Outfit,sans-serif', fontSize:14, fontWeight:700, cursor: salvando ? 'wait' : 'pointer', opacity: salvando ? 0.7 : 1 }}>{salvando ? '⏳ Salvando...' : '✓ Criar Sonho'}</button>
          </div>
        </div>
      )}
              ...
              <div style={{ background:'#fff', padding:18 }}>
                ...
                {depositing === idx ? (
                  <div style={{ display:'flex', gap:8, marginTop:12 }}>
                    ...
                    <button onClick={()=>deposit(idx)} disabled={depositando} style={{ padding:'9px 14px', borderRadius:10, border:'none', background:'#076B3E', color:'#fff', fontFamily:'Outfit,sans-serif', fontSize:12, fontWeight:700, cursor: depositando ? 'wait' : 'pointer', opacity: depositando ? 0.7 : 1 }}>{depositando ? '⏳' : '✓'}</button>
