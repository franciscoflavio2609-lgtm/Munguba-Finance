...
import { exportarRelatorioPDF } from '../../lib/exportUtils'
...
export default function ReportPanel({ onClose }) {
  ...
  // Gera uma análise real, baseada exclusivamente nos dados do próprio usuário
  const gerarAnaliseReal = () => {
    if (state.transactions.length === 0) {
      return 'Você ainda não registrou transações suficientes para gerar uma análise. Comece adicionando suas receitas e despesas para receber insights personalizados.'
    }
    const partes = []
    if (sav >= 30) partes.push(`sua taxa de poupança está em ${sav}%, acima da meta recomendada de 30%`)
    else if (sav > 0) partes.push(`sua taxa de poupança está em ${sav}%, abaixo da meta recomendada de 30%`)
    else partes.push('suas despesas superaram as receitas neste período')

    if (topCats.length > 0) {
      const [catNome, catVal] = topCats[0]
      const pct = Math.round((catVal/catTotal)*100)
      partes.push(`sua maior categoria de gasto é ${catNome}, representando ${pct}% do total de despesas`)
    }
    if (totInv > 0) partes.push(`você tem ${fmt(totInv)} investidos`)
    else partes.push('você ainda não registrou investimentos')

    return `Com base nos seus dados reais: ${partes.join('; ')}. ${bal >= 0 ? 'Seu saldo está positivo — continue assim!' : 'Seu saldo está negativo neste período — revise suas despesas variáveis.'}`
  }
  ...
  const generate = () => {
    ...
    setTimeout(()=>{
      setLoading(false)
      setAnalysis(gerarAnaliseReal())
    }, 900)
  }

  // Build chart from real user transactions only
  const buildChartData = () => {
    ...
    state.transactions.forEach(t => {
      ...
      if (t.type === 'income') months[m].inc += Number(t.val||0)
      else months[m].exp += Number(t.val||0)
    })
    ...
    return {
      labels,
      datasets:[
        {label:'Receitas',data:sorted.map(m=>months[m].inc),borderColor:'#0A8A52',backgroundColor:'rgba(10,138,82,.08)',fill:true,tension:.4,borderWidth:2.5,pointBackgroundColor:'#0A8A52',pointRadius:4},
        {label:'Despesas',data:sorted.map(m=>months[m].exp),borderColor:'#D63333',backgroundColor:'rgba(214,51,51,.06)',fill:true,tension:.4,borderWidth:2.5,pointBackgroundColor:'#D63333',pointRadius:4}
      ]
    }
  }
      ...
      <div style={{background:'linear-gradient(135deg,#022818,#076B3E)',padding:'52px 32px 24px',color:'#fff',flexShrink:0}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:18}}>
          ...
          <button onClick={() => exportarRelatorioPDF({
            ...
            dreams: state.dreams,
            mesReferencia: PERIOD_KEYS[period],
            nomeUsuario: state.user?.name || ''
          })} style={{
            display:'inline-flex',alignItems:'center',gap:8,background:'#fff',
            border:'none',color:'#076B3E',fontFamily:'Outfit,sans-serif',fontSize:13,fontWeight:700,
            padding:'8px 16px',borderRadius:10,cursor:'pointer'
          }}>📄 Exportar PDF</button>
        </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:24,display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,alignContent:'start'}}>
        ...
        <div style={{gridColumn:'1/-1'}}>
          ...
          {[
            ...
            {icon:'⚠️',label:'Atenção',txt: topCats.length > 0
              ? `Sua maior categoria de gasto é ${topCats[0][0]}, representando ${Math.round((topCats[0][1]/catTotal)*100)}% das despesas. Considere definir um limite para essa categoria.`
              : 'Registre suas despesas para receber alertas personalizados sobre seus gastos.',col:'#D97706'},
            {icon:'📈',label:'Investimentos',txt:`Patrimônio de ${fmt(totInv)}. ${totInv > 0 ? 'Considere diversificar em FIIs para renda passiva isenta de IR.' : 'Comece registrando seus primeiros investimentos na aba Investimentos.'}`,col:'#1D6FA4'},
          ].map(ins=>(
        ...
        <div style={{...card,gridColumn:'1/-1'}}>
          ...
          <div style={{position:'relative',height:200}}>
            {chartData ? (
              ...
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'100%',color:'#A8BDB5',gap:8}}>
                <span style={{fontSize:28}}>📊</span>
                <span style={{fontSize:13}}>Registre transações para visualizar o fluxo mensal</span>
              </div>
            )}
          </div>
        </div>
